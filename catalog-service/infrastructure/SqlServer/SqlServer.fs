...
module private DataAccess =
    ...
    [<Literal>] 
    let connectionString = "Server=localhost;Database=Events;User=sa;Password=Str0ngSPssw0rd02"
    ...
    let entityToEvent<'TEvent when 'TEvent :> IEvent>(e: dbSchema.dataContext.``dbo.EventEntity``) =
        let pid =
    ...
    let entityToCommand<'TCommand when 'TCommand :> ICommand>(c:dbSchema.dataContext.``dbo.CommandEntity``) =
        let pid =
    ...
    let eventToEntity<'TEvent when 'TEvent :> IEvent> (Category category) (e:EventEnvelope<'TEvent>) =
        ...
        let entity = ctx.Dbo.Event.Create(aggId, category, causId, corrId, evId, payload, typeName)
    ...
    let commandToEntity<'TCommand when 'TCommand:> ICommand> (QueueName queueName) (c:CommandEnvelope<'TCommand>) =
        ...
        let entity = ctx.Dbo.Command.Create(aggId, causId, cmdId, payload, corrId, typeName, queueName)
